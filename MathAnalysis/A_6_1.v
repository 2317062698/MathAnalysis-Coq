...
Proof.
  ...
  assert (H5 : ∀ x, F[x] = f[x] - f[a]
    - (f[b] - f[a])/(b - a)*(x - a)).
  { intro x. rewrite H3. Function_ass. }
  ...
  { unfold ContinuousClose. split; [idtac | split].
    - unfold ContinuousOpen. intros x I1.
      (* assert(F = \{\λ x y,y = f[x] - f[a]\}\ \- \{\ λ x y,y = (f[b] - f[a])/(b - a)*(x - a)\}\).
      { rewrite H3. apply AxiomI; split.
      + intros. applyAxiomII H. apply AxiomII. 
        destruct H as [x1[y1[H H']]]. 
        exists x1, y1. split; auto. split. 
        apply AxiomII. exists (f [x1] - f [a]). apply AxiomII.
        exists x1,(f [x1] - f [a]). split; auto.
        split. apply AxiomII. exists ((f [b] - f [a]) / (b - a) * (x1 - a)).
        apply AxiomII.
        exists x1,((f [b] - f [a]) / (b - a) * (x1 - a)). split; auto.   
        repeat rewrite FunValueR. rewrite H'. auto. 
      + intros. applyAxiomII H. apply AxiomII.
        destruct H as [x1[y1[H [H' [H1' H2']]]]].
        exists x1,y1. split; auto. repeat rewrite FunValueR in H2'.
        auto. }
      rewrite H.  apply Theorem4_4_2; auto. 
      + assert(\{\ λ x0 y,y = f [x0] - f [a] \}\ = 
      (\{\ λ x0 y,y = f[x0] \}\ \- \{\λ x0 y,y = f [a] \}\)).
      { apply AxiomI; split. intros.
        applyAxiomII H8. apply AxiomII. 
        destruct H8 as [x1[y1[H8 H8']]]. 
        exists x1,y1. split; auto. split.
        apply AxiomII. exists (f[x1]). apply AxiomII.
        exists x1,(f[x1]). auto. split.
        apply AxiomII. exists (f[a]). apply AxiomII.
        exists x1,(f[a]). auto. repeat rewrite FunValueR.
        auto. intros. applyAxiomII H8. 
        destruct H8 as [x1[y1[H8[H9[H10 H11]]]]].
        apply AxiomII. exists x1,y1. split; auto.
        repeat rewrite FunValueR in H11. auto.  }
      rewrite H8. apply Theorem4_4_2.
      red. apply H1 in I1. red in I1. 
      split. apply AxiomII. exists (f[x]).
      apply AxiomII'; auto. rewrite FunValueR.
      destruct I1. red in H10. red. split. QED_function. 
      destruct H10 as [H10[δ' [H11[H12 H13]]]]. 
      exists δ'. split; auto. split. intros z H14. 
      apply AxiomII. exists f[z]. apply AxiomII'; auto.
      intros. apply H13 in H14. destruct H14 as [δ[H14 H14']].
      exists δ. split; auto. intros. rewrite FunValueR.
      apply H14'; auto.
      red. split. apply AxiomII. exists f[a]. apply AxiomII'; auto.
      red. split. QED_function. exists 1. repeat split. lra.
      intros z H15. apply AxiomII. exists f[a]. apply AxiomII'; auto.
      intros. exists (1/2). split. lra.
      intros. repeat rewrite FunValueR.
      rewrite Abs_ge. lra. lra. 
     intros. repeat rewrite FunValueR. 
     apply f_x. unfold Function. intros. applyAxiomII' H11.
     applyAxiomII' H10. lra. apply AxiomII'. 
     applyAxiomII H9. repeat split. tauto. tauto.
     repeat rewrite FunValueR; auto. 
     + admit.
     + intros. repeat rewrite FunValueR.
       apply f_x. unfold Function. intros. applyAxiomII' H9.
     applyAxiomII' H10. lra. apply AxiomII'. 
     applyAxiomII H8. repeat split. tauto. tauto.
     repeat rewrite FunValueR; auto.
   - 
     } 
